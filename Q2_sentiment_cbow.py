...
context_size = 2    
model_cbow = CBOW(context_size, vocab_size)
model_cbow.load_state_dict(torch.load('models/model_cbow-14_vector_size100.pth'))
model_cbow.eval() 
embeddings_cbow = model_cbow.embedding_layer.weight.to('cpu').data.numpy()
...
for epoch in (range(50)):
    ...
    for step in tqdm(range(no_of_steps)):
        x,y = generate_batch(train, sent_length , embedding_size, batch_size, step, embeddings_cbow, dict_word_to_index )
    ...
    val_accuracy = 0
    for step in tqdm(range(no_of_steps)):
        x_val ,y_val = generate_batch(val, sent_length , embedding_size, batch_size, step, embeddings_cbow, dict_word_to_index )
    ...
    torch.save(net.state_dict(), './models/model_sentiment_cbow-'+str(epoch)+'vector_size-'+str(embedding_size)+'.pth')
...
val_accuracy = 0
for step in tqdm(range(no_of_steps)):
    x_val ,y_val = generate_batch(test, sent_length , embedding_size, batch_size, step, embeddings_cbow, dict_word_to_index )
