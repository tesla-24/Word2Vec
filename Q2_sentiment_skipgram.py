...
model_skipgram = skipgram(vocab_size)
model_skipgram.load_state_dict(torch.load('models/model_skipgram-60_vector_size100.pth'))
model_skipgram.eval() 

embeddings  = model_skipgram.input_embeddings.weight.to('cpu').data.numpy()
...
for epoch in (range(50)):
    ...
    for step in tqdm(range(no_of_steps)):
        x,y = generate_batch(train, sent_length , embedding_size, batch_size, step, embeddings, dict_word_to_index )
        if torch.cuda.is_available():
    ...
    val_accuracy = 0
    for step in tqdm(range(no_of_steps)):
        x_val ,y_val = generate_batch(val, sent_length , embedding_size, batch_size, step, embeddings, dict_word_to_index )
    ...
    torch.save(net.state_dict(), './models/model_sentiment_skipgram-'+str(epoch)+'vector_size-'+str(embedding_size)+'.pth')
...
val_accuracy = 0
for step in tqdm(range(no_of_steps)):
    x_val ,y_val = generate_batch(test, sent_length , embedding_size, batch_size, step, embeddings, dict_word_to_index )
